...
extern "C" void v_hscaler(volatile void *, volatile void *, volatile void *, volatile void *, volatile void *, volatile void *, volatile void *, volatile void *, volatile void *, volatile void *, volatile void *, volatile void *, volatile void *, short*, short*, volatile void *, volatile void *, volatile void *, volatile void *, volatile void *, volatile void *, volatile void *);
...
auto* ss_axis_video_V_dest_V = bcsim::createStream((hls::stream<char>*)__xlx_apatb_param_s_axis_video_V_dest_V);
  ...
  vector<sc_bv<16> >__xlx_hfltCoeff__tmp_vec;
  for (int j = 0, e = 384; j != e; ++j) {
    sc_bv<16> _xlx_tmp_sc;
    _xlx_tmp_sc.range(7, 0) = ((char*)__xlx_apatb_param_hfltCoeff)[j*2+0];
    _xlx_tmp_sc.range(15, 8) = ((char*)__xlx_apatb_param_hfltCoeff)[j*2+1];
    __xlx_hfltCoeff__tmp_vec.push_back(_xlx_tmp_sc);
  }
  int __xlx_size_param_hfltCoeff = 384;
  int __xlx_offset_param_hfltCoeff = 0;
  int __xlx_offset_byte_param_hfltCoeff = 0*2;
  short* __xlx_hfltCoeff__input_buffer= new short[__xlx_hfltCoeff__tmp_vec.size()];
  for (int i = 0; i < __xlx_hfltCoeff__tmp_vec.size(); ++i) {
    __xlx_hfltCoeff__input_buffer[i] = __xlx_hfltCoeff__tmp_vec[i].range(15, 0).to_uint64();
  }
  // Collect __xlx_phasesH__tmp_vec
  vector<sc_bv<9> >__xlx_phasesH__tmp_vec;
  for (int j = 0, e = 1920; j != e; ++j) {
    sc_bv<9> _xlx_tmp_sc;
    _xlx_tmp_sc.range(7, 0) = ((char*)__xlx_apatb_param_phasesH)[j*2+0];
    _xlx_tmp_sc.range(8, 8) = ((char*)__xlx_apatb_param_phasesH)[j*2+1];
    __xlx_phasesH__tmp_vec.push_back(_xlx_tmp_sc);
  }
  int __xlx_size_param_phasesH = 1920;
  int __xlx_offset_param_phasesH = 0;
  int __xlx_offset_byte_param_phasesH = 0*2;
  short* __xlx_phasesH__input_buffer= new short[__xlx_phasesH__tmp_vec.size()];
  for (int i = 0; i < __xlx_phasesH__tmp_vec.size(); ++i) {
    __xlx_phasesH__input_buffer[i] = __xlx_phasesH__tmp_vec[i].range(8, 0).to_uint64();
  }
...
// print __xlx_apatb_param_hfltCoeff
  sc_bv<16>*__xlx_hfltCoeff_output_buffer = new sc_bv<16>[__xlx_size_param_hfltCoeff];
  for (int i = 0; i < __xlx_size_param_hfltCoeff; ++i) {
    __xlx_hfltCoeff_output_buffer[i] = __xlx_hfltCoeff__input_buffer[i+__xlx_offset_param_hfltCoeff];
  }
  for (int i = 0; i < __xlx_size_param_hfltCoeff; ++i) {
    ((char*)__xlx_apatb_param_hfltCoeff)[i*2+0] = __xlx_hfltCoeff_output_buffer[i].range(7, 0).to_uint();
    ((char*)__xlx_apatb_param_hfltCoeff)[i*2+1] = __xlx_hfltCoeff_output_buffer[i].range(15, 8).to_uint();
  }
// print __xlx_apatb_param_phasesH
  sc_bv<9>*__xlx_phasesH_output_buffer = new sc_bv<9>[__xlx_size_param_phasesH];
  for (int i = 0; i < __xlx_size_param_phasesH; ++i) {
    __xlx_phasesH_output_buffer[i] = __xlx_phasesH__input_buffer[i+__xlx_offset_param_phasesH];
  }
  for (int i = 0; i < __xlx_size_param_phasesH; ++i) {
    ((char*)__xlx_apatb_param_phasesH)[i*2+0] = __xlx_phasesH_output_buffer[i].range(7, 0).to_uint();
    ((char*)__xlx_apatb_param_phasesH)[i*2+1] = __xlx_phasesH_output_buffer[i].range(8, 8).to_uint();
  }
